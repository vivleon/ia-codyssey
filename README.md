...
이미지 레이어는 생성된 뒤 읽기 전용이며 컨테이너의 변경은 별도 writable layer에 기록됩니다. 다만 `nginx:alpine` 같은 태그는 변경 가능한 포인터이므로 나중에 같은 태그를 pull하거나 build하면 다른 이미지 ID를 가리킬 수 있습니다. 이미 만들어진 컨테이너는 생성 당시 이미지 ID를 계속 사용하지만, 다음 빌드의 재현성을 보장하려면 이 프로젝트처럼 digest를 고정해야 합니다. [다중 컨테이너·포트 로그](logs/multi-container-ports.txt)에서 서로 다른 두 컨테이너가 같은 이미지 ID로 생성됐음을 확인할 수 있습니다.
...
Docker 볼륨은 컨테이너 삭제와 분리되어 있지만 볼륨 자체 삭제, 디스크 장애 또는 런타임 초기화까지 막아 주는 백업은 아닙니다. 이 저장소에서는 읽기 전용으로 연결한 원본 볼륨을 tar 파일로 백업하고, 원본 컨테이너와 원본 볼륨을 실제로 삭제한 다음 새 빈 볼륨에 복원했습니다. 별도 읽기 전용 컨테이너에서 내용과 SHA-256뿐 아니라 비기본 mode `640`, UID `1234`, GID `2345`가 원본과 같은지도 확인했습니다.
...
수동으로 확인할 때는 [이미지 빌드](#114-이미지-빌드), [컨테이너 실행](#115-컨테이너-실행), [포트 매핑](#12-포트-매핑)의 명령을 순서대로 사용합니다. 고정 이름이나 포트가 이미 존재하면 용도를 확인하고 다른 값을 선택하며, 실습 후에는 [Docker 기본 운영 명령](#10-docker-기본-운영-명령)에 따라 자신이 만든 자원만 정리합니다.

---

## 19. 핵심 학습 내용

이 장에서는 앞에서 설명한 개념을 반복하지 않고 기준 설명으로 바로 연결합니다.

| 학습 개념 | 기준 설명 |
|---|---|
| 절대 경로·상대 경로 | [5.1~5.4 경로 선택 기준](#51-절대-경로) |
| `r/w/x`, `755`, `644` | [6. 파일 및 디렉토리 권한](#6-파일-및-디렉토리-권한) |
| 이미지·컨테이너 | [10.1 이미지와 컨테이너의 차이](#101-이미지와-컨테이너의-차이) |
| 커스텀 이미지 | [11. 커스텀 Docker 이미지](#11-커스텀-docker-이미지) |
| 포트 매핑 | [12. 포트 매핑](#12-포트-매핑) |
| 바인드 마운트·볼륨 | [13. 바인드 마운트](#13-바인드-마운트), [14. Docker 볼륨 영속성](#14-docker-볼륨-영속성) |
| Git·GitHub | [15.1 Git과 GitHub의 차이](#151-git과-github의-차이) |
...
| 평가 질문 | 바로 말할 답변 | 상세 설명·증거 |
|---|---|---|
| 프로젝트 디렉토리를 어떤 기준으로 구성했는가? | 빌드 입력(`app/`), 런타임 입력(`bind-app/`), 재현 절차(`scripts/`), 결과 증거(`logs/`, `screenshots/`)를 역할별로 분리하고 README를 진입점으로 삼았습니다. | [프로젝트 구조와 설계 기준](#4-프로젝트-구조), [구조 검증 로그](logs/project-structure.txt) |
| 포트와 볼륨 설정을 어떻게 재현 가능하게 정리했는가? | Dockerfile과 실행 명령으로 설정을 문서화하고 `scripts/verify.sh`로 빌드·포트·HTTP·볼륨 검증을 자동화했습니다. 실행별 고유 이름과 label을 사용해 기존 자원과의 충돌도 줄였습니다. | [자동 검증](#181-권장-자동-검증), [스크립트](scripts/verify.sh), [원본 로그](logs/automated-verification.txt) |

### 22.3 항목 3: 핵심 기술 원리 적용

| 평가 질문 | 바로 말할 답변 | 상세 설명·증거 |
|---|---|---|
| 이미지와 컨테이너의 차이는? | 이미지는 `build`로 만드는 읽기 전용 실행 템플릿이고, 컨테이너는 이미지를 `run`한 실행 인스턴스입니다. 애플리케이션 변경은 재빌드하고, 유지할 데이터는 볼륨이나 바인드 마운트로 분리합니다. | [이미지와 컨테이너의 차이](#101-이미지와-컨테이너의-차이) |
| 포트 매핑이 왜 필요한가? | 컨테이너는 별도 네트워크 네임스페이스를 사용하므로 내부 포트가 호스트에 자동 공개되지 않습니다. 이 프로젝트는 `127.0.0.1:8080:80`으로 필요한 경로만 연결했습니다. | [포트 매핑](#12-포트-매핑), [실행 로그](logs/image-build-run.txt) |
| 절대·상대 경로는 언제 선택하는가? | 저장소 내부는 복제 위치에 독립적인 상대 경로를 쓰고, 정확한 마운트 위치처럼 기준이 고정돼야 할 때는 절대 경로를 사용합니다. | [경로 선택 기준](#51-절대-경로) |
| `755`, `644`는 어떤 규칙인가? | `r=4`, `w=2`, `x=1`을 더한 값을 소유자·그룹·기타 사용자 순서로 적습니다. `755`는 `rwxr-xr-x`, `644`는 `rw-r--r--`입니다. | [권한 설명](#6-파일-및-디렉토리-권한), [권한 로그](logs/permissions.txt) |

### 22.4 항목 4: 심층 인터뷰

| 평가 질문 | 바로 말할 답변 | 상세 설명·증거 |
|---|---|---|
| 호스트 포트가 이미 사용 중이면? | 오류의 host port를 확인하고 `lsof`와 Docker 목록으로 점유 주체를 찾습니다. 서비스 영향을 확인한 뒤 불필요한 자원만 정리하거나 다른 빈 포트를 사용하고 HTTP 응답까지 재검증합니다. | [포트 충돌 진단](#163-호스트-포트가-이미-사용-중이라-컨테이너-실행-실패), [원본 로그](logs/port-conflict.txt) |
| 컨테이너 삭제 후 데이터가 사라졌다면? | writable layer가 아닌 볼륨이나 바인드 마운트에 데이터를 저장하고, 중요한 데이터는 별도 백업과 정기 복원 시험으로 보호합니다. | [볼륨 영속성·백업](#14-docker-볼륨-영속성), [복원 로그](logs/volume-backup-restore.txt) |
| 가장 어려웠던 지점과 해결 과정은? | 같은 host port 사용으로 종료 코드 `125`가 발생했습니다. 포트 점유를 가설로 세우고 `lsof`, `docker ps`, `docker port`로 확인한 뒤 기존 서비스를 유지하면서 대체 포트로 실행해 HTTP `200`을 확인했습니다. | [트러블슈팅 사례 3](#163-호스트-포트가-이미-사용-중이라-컨테이너-실행-실패), [원본 로그](logs/port-conflict.txt) |

### 22.5 추가로 자주 묻는 질문

- `nginx:alpine` 선택 이유: [커스텀 적용 내용](#112-커스텀-적용-내용)
- 바인드 마운트와 볼륨의 차이: [바인드 마운트와 볼륨 비교](#141-바인드-마운트와-볼륨-비교)
- `attach`와 `exec`의 차이: [컨테이너 종료·유지 관찰](#91-run-attach-exec-차이)
...
- 검증하지 않은 범위는? 저장된 실측 결과는 Docker Desktop 환경입니다. [서울캠퍼스의 OrbStack 대안](#2-실행-환경), 특정 GitHub OAuth 방식, 실제 외부 암호화 백업 운영까지 검증했다고 주장하지 않습니다.

---

## 23. 보너스 과제

### 23.1 Docker Compose 기초: 단일 서비스
...
기본 포트 연결은 호스트 `127.0.0.1:8090`에서 컨테이너 `8080`입니다. 상태·로그·종료 명령은 [Compose 운영 명령 루틴](#233-compose-운영-명령-루틴)을 따릅니다. 실제 검증에서는 기존 서비스와의 충돌을 피하려고 실행 전용 host port `18090`을 주입했고 HTTP `200`과 `X-App-Mode: single-verified`를 확인했습니다.

### 23.2 Docker Compose 멀티 컨테이너
...
시작 후 상태·로그·종료는 아래 공통 운영 루틴을 사용합니다.

### 23.3 Compose 운영 명령 루틴
